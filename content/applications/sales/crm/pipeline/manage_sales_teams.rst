...
.. note::
   When using rule-based assignment, Odoo automatically limits the maximum number of leads that an
   individual teammember can be be assigned in a day. This limit is determined by the number in the
   :guilabel:`Capacity` field and is equal to that number divided by 30 (for the number of days in
   the period.) **This may result in leads going unassigned if all sales team member's daily
   capacity has been met.**

   For example, Sales Team Member John Doe has a :guilabel:`Capacity` maximum of 120 leads in a
   30-day period. This means that the database automatically assigns John a maximum of 4 leads per
   day. If John were the only member of the sales team and 10 leads came in in a single day, only 4
   would be assigned to him. The other 6 would go unassigned.
