...
not fill anything in this view:
...
The :guilabel:`ARCA POS System` is a field visible only in the **Sales** journals and defines the
type of ARCA POS to be used to manage the transactions for which the journal is created.
...
.. note::
   The VAT book report can be exported as a `.zip` file by selecting it in the drop-down menu in the
   top-left corner.
...
#. Generate Renewal Request: In the :guilabel:`ARCA Web Services` section, click
   ...
   signing request) file to use in |ARCA| portal to request the certificate.
#. Obtain Certificate (ARCA): Access the |ARCA| portal and follow the instructions described in
   ...
   a certificate.
#. Upload Certificate and Private Key (Odoo): Upload the certificate to Odoo by clicking the
