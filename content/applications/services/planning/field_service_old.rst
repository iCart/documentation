...
:guilabel:`Field Service`, and click :guilabel:`Save`. The following options are then available
in the :guilabel:`Field Service` section:

- :guilabel:`Billing`: to invoice customers for time spent on and :ref:`materials
  <planning/field-service/products>` used during onsite interventions. Once enabled, the
  following options are automatically enabled:

  - :guilabel:`Products`: to add the :ref:`materials <planning/field-service/products>` used during
    interventions on field service shifts and bill them to the customer.
  - :guilabel:`Customer Report`: to automatically generate a report for the customer at the end of
    the intervention, collect their signature, and email them a copy.
  ...
  .. tip::
     Enable :guilabel:`Hide prices on customer report` under :guilabel:`Products` to

- :guilabel:`Worksheets`: to complete :ref:`worksheets <planning/field-service/worksheets>` using
  custom templates.
- :guilabel:`Quotations`: to create new quotations directly from field service shifts.
- :guilabel:`Geolocation`: to track technicians' locations at intervention start and completion.
