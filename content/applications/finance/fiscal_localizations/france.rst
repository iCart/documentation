...
.. note::
   Make sure to :doc:`install </applications/general/apps_modules>` the :guilabel:`France -
   Accounting Reports` (`l10n_fr_reports`) module.
...
.. note::
   ...
   France) and for those receiving copyright royalties (DA) or inventor's fees (DI).
...
To submit the EDI DAS2 report, follow these steps:
...
#. In the :guilabel:`DAS2 Report Generation` window, update the :guilabel:`Point of Contact` field
   if necessary, and select the contact responsible for directly sending the report to |DGFiP|.
...
.. tip::
   ...
   Alternatively, it can also be submitted following these steps:
   ...
   #. In the :guilabel:`DAS2 Report` window, update the :guilabel:`Point of Contact` field if
      necessary, and select the contact responsible for directly sending the report to |DGFiP|, and
      click :guilabel:`Send DAS2 Report`.
...
#. Go to :menuselection:`Accounting --> Reporting --> Fiscal Declaration`. In the :guilabel:`Fiscal
   Declaration` view, click :guilabel:`Returns`.
#. Remove the :guilabel:`To do` filter in the Tax Return list view to display the :guilabel:`Fiscal
   Declaration` line.
#. The :guilabel:`Fiscal Declaration` line includes:
   ...
   - Action steps, such as :guilabel:`Review` and :guilabel:`Submit`, which turn green when
     completed.
   - A :guilabel:`Fiscal Declaration` button to open the :guilabel:`Fiscal Declaration`.
   - A :icon:`fa-ellipsis-v` :guilabel:`(vertical ellipsis)` menu for additional options.
